...
def drn_attention(pretrained=False, **kwargs):
    model = DRN(Bottleneck, [1, 1, 3, 4, 6, 3, 1, 1], arch='D', **kwargs)
    if pretrained:
        pretrained_dict = model_zoo.load_url(model_urls['drn-d-54'])
        model_dict = model.state_dict()

        # filter out unnecessary dict in model dict which not belong to model_dict
...
if __name__ == "__main__":
    ...
    model = drn_attention(pretrained=True, num_classes=5).cuda()
    model(Variable(x).cuda())